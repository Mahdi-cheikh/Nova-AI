  ...
  try {
    ...
    for (const entry of body.entry ?? []) {
      for (const change of entry.changes ?? []) {
        ...
        const { data: biz } = await sb.from("businesses").select("id, ai_receptionist_enabled, name").eq("phone", "+" + recipientPhone).maybeSingle();
        if (!biz) continue;
        ...
        for (const msg of value.messages ?? []) {
          ...
          // run Claude Vision OCR, parse tests, then forward as text so the lab
          // booking flow picks it up.
          if (!text && msg.type === "image" && msg.image?.id && biz.type === "laboratoire") {
            try {
              const metaRes = await fetch(`https://graph.facebook.com/v20.0/${msg.image.id}`, {
                headers: { Authorization: `Bearer ${WA_ACCESS_TOKEN}` },
              });
              const metaJson = await metaRes.json();
              if (metaJson.url) {
                ...
                // Run Claude Vision OCR to extract test names
                const b64 = btoa(String.fromCharCode(...imgBuf.slice(0, 5_000_000)));
                const ocrRes = await fetch("https://api.anthropic.com/v1/messages", {
                  ...
                  body: JSON.stringify({
                    ...
                    messages: [{ role: "user", content: [
                      ...
                      { type: "text", text: "This is a medical/lab prescription. Extract: prescriber_name, prescription_date (YYYY-MM-DD), and an array of tests (each as a short standard name). Reply with valid JSON ONLY: {\"prescriber_name\":\"\",\"prescription_date\":\"\",\"tests\":[\"\"]}. No prose." },
                    ] }],
                  }),
                });
                const ocrJson = await ocrRes.json();
                const ocrText = ocrJson?.content?.[0]?.text || "{}";
                const parsed  = JSON.parse((ocrText.match(/\{[\s\S]*\}/) || ["{}"])[0]);
                ...
                // Forward to classify-message as if the patient typed the test list
                text = (parsed.tests || []).join(", ") || "I have a prescription";
                console.log("Prescription parsed:", text);
              }
            } catch (e) {
